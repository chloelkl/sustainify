...
import { Box, Input, IconButton, Card, CardMedia, CardContent, Typography } from '@mui/material';
...
function Forum() {
    ...
    const searchForums = () => {
        http.get(`/forum?search=${search}`)
            .then((res) => {
                if (Array.isArray(res.data)) {
                    setForumList(res.data);
                    console.log('Searched Forums:', res.data); // Log the searched forums
                } else {
                    setForumList([]);
                    console.error('Expected array but received:', res.data);
                }
    ...
    const ForumItems = forumList.map((item) => {
        console.log('Rendering Forum Item:', item); // Log each forum item
        if (!item.userId || !item.User?.username) {
            console.error("Invalid forum item data:", item);
